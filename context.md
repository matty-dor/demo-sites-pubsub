...
*Last aligned with implementation in this workspace as of **May 14, 2026**: May 8–9 v2 sandbox baseline; plus **May 12–14** — image preview flush layout, Experiences v2 summary actions + `var(--text)` live canvas + removed live lede, Dynamic v2 without Content source radios + `{{api_value}}`, **`timestamp`** schema/publish/validation pipeline, **`injectTriggerTimestamps`** (client + server), and **`mock-event-trigger-btn`** press feedback on Trigger and v2 Refresh/Reset.*
